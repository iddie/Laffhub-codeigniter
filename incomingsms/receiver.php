	...
	try
	{
		...
		{ 			
		 	...
		 	$file = fopen('messages.txt',"a"); fwrite($file, "MSISDN=".$msisdn." => SHORTCODE=".$shortcode." => Network=".$network." => Date=".$reqdate." => MESSAGE=".$message.PHP_EOL); fclose($file);
		  	...
		  	#Check if it is subscription request
		  	$requestplan=IsSubscriptionRequest($message,$shortcode);
#$file = fopen('requestplan.txt',"a"); fwrite($file, $requestplan.PHP_EOL); fclose($file);			
			if ($requestplan<>'')
			{
			  #Check if there is an existing request
				if (trim(strtolower($requestplan))=='lol')#Direct
				{						
					$ret=CheckForBlackList($network,$msisdn,$db); #Check if number has been blacklisted
					...
					}elseif ($shortcode=='2001') #N20/Daily
					{#DAY		  
				...
				}elseif (trim(strtolower($requestplan))=='confirm')#Confirm subscription request - Subscribe
				{
					#check if there is active request
					$res=GetExistingRequestInfo($network,$msisdn,$shortcode,$db);
					
					if ($res<>false)#There is request - subscribe
					{
						$plan=$res; 
						...
						if (($ret==true) and ($shortcode=='2001'))#Blaclisted Number
						{
							...
							$ret=SendAirtelSms($msisdn,$msg,$db);
						}elseif (($shortcode=='2001') and (strtoupper($plan)=='WEEKLY'))#N100/Weekly
					 	{#WEEKLY
							$subscription_msg=''; $amount=''; $insufficent_balance_msg=''; $wrong_keyword_msg='';
							$subscriptiondays=''; $amount=''; $autobilling='1'; $videos_cnt_to_watch=''; $email='';
							$subscriptionstatus='1'; $watched='0';
				...
				#$file = fopen('aaa_SUB.txt',"a"); fwrite($file, "\n\nSUCCESS\nStatus=".$ret['Status']."\nResult=".$result."MSISDN=".$msisdn."\nPlan=".$plan."\nAmount=".$amount.PHP_EOL); fclose($file);
					 	...
					 	}elseif (($shortcode=='2001') and (strtoupper($plan)=='DAILY'))#N20/Daily
						{#DAY		  
							$subscription_msg=''; $amount=''; $insufficent_balance_msg=''; $wrong_keyword_msg='';
							$subscriptiondays=''; $amount=''; $autobilling='1'; $videos_cnt_to_watch=''; $email='';
							$subscriptionstatus='1'; $watched='0';
				...
				#$file = fopen('aaa_SUB.txt',"a"); fwrite($file, "\n\nSUCCESS\nStatus=".$ret['Status']."\nResult=".$result."MSISDN=".$msisdn."\nPlan=".$plan."\nAmount=".$amount.PHP_EOL); fclose($file);
					  	...
					  	}elseif (($shortcode=='2001') and (strtoupper($plan)=='MONTHLY'))#N200/Monthly
					 	{#MONTH		  
							$subscription_msg=''; $amount=''; $insufficent_balance_msg=''; $wrong_keyword_msg='';
							$subscriptiondays=''; $amount=''; $autobilling='1'; $videos_cnt_to_watch=''; $email='';
							$subscriptionstatus='1'; $watched='0';
					  	...
					  	}elseif (($shortcode=='2001') and (strtoupper($plan)=='UNLIMITED'))#N500/UNLIMITED
                        {#UNLIMITED
                            $subscription_msg = '';
                            $amount = '';
                        ...
                        elseif (($shortcode=='2001') and (strtoupper($plan)=='TRY'))
                        {
                            $subscription_msg=''; $amount=''; $insufficent_balance_msg=''; $wrong_keyword_msg='';
                            $subscriptiondays=''; $amount=''; $autobilling='1'; $videos_cnt_to_watch=''; $email='';
                            $subscriptionstatus='1'; $watched='0';
				...
				}elseif (trim(strtolower($requestplan))=='out')
				{
	...
	#LogDetails($Name,$Activity,$Username,$logdate,$ip,$host,$Operation,$LogID,$db)					
				...
				}elseif (trim(strtolower($requestplan))=='help')
				{
					$subscription_msg='Text DAY to 2001 for Daily Plan, COMEDY to 2001 for Weekly plan, MONTH to 2001 for Monthly Plan, UNLIMITED to 2001 for unlimited Plan,OUT to 2001 to unsubscribe';
				...
				else#Create subscription request
				{
					...
					if ($res==false)#Does not exist
					{
						#save request
						CreateSubscriptionRequest($network,$msisdn,$requestplan,$shortcode,$reqdate,$db);
						
						$subscription_msg='Your request to subscribe to LaffHub '.strtoupper($requestplan).' plan has been received. Text 1 to 2001 to confirm. Subscription request expires after 1 hour if not confirmed.';
							  
			  			$ret=SendAirtelSms($msisdn,$subscription_msg,$db);
					}else #Exists
					{
						#send message that confirmation is being awaited for existing request
						$subscription_msg='You have already requested for subscription on LaffHub portal. Text 1 to 2001 to confirm. Subscription request expires after 1 hour if not confirmed.';
		...
		{
			$file = fopen('wrong_2001_msg.txt',"a"); fwrite($file, "MSISDN=".$_GET["msisdn"]." => SHORTCODE=".$_GET["shortcode"]." => Network=".$network." => Date=".date('Y-m-d H:i:s')." => MESSAGE=".$_GET["message"].PHP_EOL); fclose($file);
		}#End if ((isset($_GET["msisdn"])) and (isset($_GET["message"])) and (isset($_GET["shortcode"]))) 
	}catch(Exception $e) {
	...
	#********************************************************************************************************
	#********************************************************************************************************
	function GetExistingRequestInfo($network,$msisdn,$shortcode,$db)
	{
		#Save to DB				
		$sql = "SELECT plan FROM subscription_requests WHERE (TRIM(msisdn)='".$db->escape_string($msisdn)."') AND (TRIM(network)='".$db->escape_string($network)."') AND (TRIM(shortcode)='".$db->escape_string($shortcode)."')";
		...
		if ($result->num_rows > 0 )
		{
			...
			if ($row['plan']) return trim($row['plan']); else return false;
		}else
		{
			return false;
		}
	...
	function CheckIfThereisExistingRequest($network,$msisdn,$plan,$shortcode,$db)
	{
		#Save to DB				
		$sql = "SELECT * FROM subscription_requests WHERE (TRIM(msisdn)='".$db->escape_string($msisdn)."') AND (TRIM(network)='".$db->escape_string($network)."') AND (TRIM(plan)='".$db->escape_string($plan)."') AND (TRIM(shortcode)='".$db->escape_string($shortcode)."')";
	...
	function CreateSubscriptionRequest($network,$msisdn,$plan,$shortcode,$requestdate,$db)
	{
		$db->autocommit(FALSE);
				
		$sql='INSERT INTO subscription_requests (network,msisdn,shortcode,plan,requestdate) VALUES (?,?,?,?,?)';										
		$stmt = $db->prepare($sql);#
		
		if ($stmt === false) trigger_error('Wrong SQL: '.$sql.' Error: '.$db->error, E_USER_ERROR);
		
		$plan=$db->escape_string($plan);
		$msisdn=$db->escape_string($msisdn);
		$shortcode=$db->escape_string($shortcode);
		
		#Bind parameters. TYpes: s = string, i = integer, d = double,  b = blob#
		$stmt->bind_param('sssss',$network,$msisdn,$shortcode,$plan,$requestdate);				
		$stmt->execute();			
		$db->commit();
	...
	function IsSubscriptionRequest($message,$shortcode)#Returns Plan if it a subscription request
	{
		if (!$message) return '';
		
		$m=trim(strtoupper($message));
		...
		}elseif ((trim($shortcode)=='2001') and (($m=='COMEDY') or ($m=='YES') or ($m=='OKAY') or ($m=='OK') or ($m=='DAY') or ($m=='MONTH') or ($m=='UNLIMITED') or ($m=='TRY')))
		{
			...
			{
				return 'Daily';
			}elseif (($m=='COMEDY') or ($m=='YES') or ($m=='OKAY') or ($m=='OK'))
			{
				return 'Weekly';
			}elseif ($m=='MONTH')
			{
	...
	function SubscribeAirtelUser($email,$network,$msisdn,$plan,$subscriptiondays,$amount,$autobilling,$subscribe_date,$exp_date,$watched,$videos_cnt_to_watch,$subscriptionstatus,$transid,$cptransid,$subscription_message,$errorCode,$errorMessage,$subscription_status,$subscriptionId,$db)
	{
		...
		{#- array('Status' => 'OK','errorCode' => '','errorMessage' =>'', 'TransId' => $transid,'cpTransId' => $cptransid);
			...
			#Remove from subscription_requests
			$sql = "SELECT * FROM subscription_requests WHERE (TRIM(network)='".$db->escape_string($network)."') AND (TRIM(msisdn)='".$db->escape_string($msisdn)."')";
			
			if(!$query = $db->query($sql)) die('There was an error running the query ['.$db->error.']');
			
			if ($query->num_rows > 0 )#There is active subscription
			{
				$sql = "DELETE FROM subscription_requests WHERE (TRIM(network)='".$db->escape_string($network)."') AND (TRIM(msisdn)='".$db->escape_string($msisdn)."')";
				$db->query($sql);
			}
			################## END REMOVE	
