...
#$file = fopen('requestplan.txt',"a"); fwrite($file, $requestplan.PHP_EOL); fclose($file);			
			if ($requestplan<>'')
			{
			  #Check if there is an existing request
				if (trim(strtolower($requestplan))=='confirm')#Confirm subscription request - Subscribe
				{
	...
	function IsSubscriptionRequest($message,$shortcode)#Returns Plan if it a subscription request
	{
		...
		if ((trim($shortcode)=='2001') and (($m=='COMEDY') or ($m=='YES') or ($m=='OKAY') or ($m=='OK') or ($m=='DAY') or ($m=='MONTH') or ($m=='UNLIMITED')))
		{
			if ($m=='DAY')
			{
				return 'Daily';
