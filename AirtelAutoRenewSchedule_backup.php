...
function AutoRenewSubscriptions($network,$db)
{
	...
	$sql="SELECT DISTINCT(plan) AS plan FROM subscriptions WHERE (TRIM(network)='".$network."') AND ((DATE_FORMAT(`exp_date`,'%Y-%m-%d %H:%i:%s') <= '".$dt."') OR (TRIM(network)='".$db->escape_string($network)."') AND (subscriptionstatus=0))";
	...
	$sql="SELECT msisdn,plan,duration,amount,email,subscribe_date,exp_date,videos_cnt_to_watch,autobilling FROM subscriptions WHERE (TRIM(network)='".$db->escape_string($network)."') AND ((DATE_FORMAT(`exp_date`,'%Y-%m-%d %H:%i:%s') <= '".$dt."') OR (TRIM(network)='".$db->escape_string($network)."') AND (subscriptionstatus=0))";
	...
	while($srow = $qry->fetch_assoc()):
		$amount=''; 
		$subscriptiondays=''; $videos_cnt_to_watch=''; $email='';
		$subscriptionstatus=''; $watched='0'; $autobilling=1; $msisdn='';
		...
		if (trim($msisdn) <> '')
		{
			...
			if ($black==false)
			{
				...
				if ($rt==false)
				{
					...
					if ($autobilling==1)
					{#echo '<br>'.$plan.' => '.$msisdn.' => '.$arr[$plan].'<br>';
						try
						{				
							$ret=''; $watched=0; $maxwatch=0; $expdt=''; $activeplan=''; $flag=false; $ex=''; $subscriptionstatus=1;
			...
			#Array ( [status] => Failure [transactionId] => twss_54e21d846749779 [error] => Array ( [errorCode] => OL404 [errorMessage] => Insufficient Balance.#~#95.45 ) [cpTransactionId] => 20170617155509_2348083964929 )
							...
							if (isset($ret))
							{
								if (strtoupper($ret['Status']) <> 'FAILED')#Success
								{
									...
									{									
										...
										#Log
										$file = fopen('airtel_renewal_success.txt',"a"); fwrite($file, date('Y-m-d H:i:s').",MSISDN: ".$msisdn.",Plan: ".$plan.",Status: ".$ret["Status"].",Trans. Id: ".$ret["TransId"].",CP Trans. Id: ".$ret["cpTransId"].",".$passed.PHP_EOL); fclose($file);
								...
								}else
								{
									...
									$file = fopen('airtel_renewal_error.txt',"a"); fwrite($file, date('Y-m-d H:i:s').",MSISDN: ".$msisdn.",Plan: ".$plan.",Status: ".$ret["Status"].",Error Code: ".$ret["errorCode"].",Error Message: ".$ret["errorMessage"].",".$failed.PHP_EOL); fclose($file);
									...
									if (trim(strtoupper($ret["Status"]))=='FAILED')
									{
										if ($isnew==1)#Insert Into Failed Activations
										{
											...
											if ($query->num_rows > 0 )
											{
												...
												{
													$file = fopen('mysqli_error_log.txt',"a"); fwrite($file, date('Y-m-d H:i:s').",AirtelAutoRenewSchedule.php,AutoRenewSubscriptions,ERROR=>".$db->error.PHP_EOL); fclose($file);
												}
							...
							}else
							{
								...
								$file = fopen('airtel_renewal_error.txt',"a"); fwrite($file, date('Y-m-d H:i:s').",MSISDN: ".$msisdn.",Status: ".$ret["Status"].",Error Code: ".$ret["errorCode"].",Error Message: ".$ret["errorMessage"].",".$failed.PHP_EOL); fclose($file);
						...
						{#Array ( [Status] => FAILED [errorCode] => FFF [errorMessage] => Could not connect to host )
