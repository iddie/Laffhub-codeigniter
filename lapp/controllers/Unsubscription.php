...
class Unsubscription extends CI_Controller {
	...
	public function GetReport()
	{
		...
		$sql="SELECT network,DATE_FORMAT(optout_date,'%d %b %Y') AS sdate, lastplan,COUNT(msisdn) AS Total FROM optouts WHERE (TRIM(network)='".$network."') AND (DATE_FORMAT(optout_date,'%Y-%m-%d') BETWEEN '".$startdate."' AND '".$enddate."') GROUP BY DATE_FORMAT(optout_date,'%Y-%m-%d'),lastplan ORDER BY optout_date DESC;";
...
#$file = fopen('aaa.txt',"w"); fwrite($file, $sql."\n"); fclose($file);	
	...
	public function PrintReport()
	{
		...
		if ($query->num_rows() > 0)
		{
			...
			$xls_file='unsubscriptionreport.xls';
			...
			$objPHPExcel->getProperties()->setCreator("Eastwinds ICT")
								 ...
								 ->setKeywords("Unsubscription")
								 ->setCategory("Unsubscription");
			...
			$objPHPExcel->getActiveSheet()->setCellValue('A5', "S/N")
								  ...
								  ->setCellValue('C5', "Unsubscription Date")
			...
			$objPHPExcel->setActiveSheetIndex(0);
			$objPHPExcel->getActiveSheet()->setTitle('Unsubscription Records');
		...
		{
			$ret="There is no unsubscription record for the selected date.";
		}
	...
	{		
		...
		if ($_SESSION['username'])
		{
			...
			$this->load->view('unsubscription_view',$data);
		}else
		{
			redirect("Home");
		}
