...
class Home extends CI_Controller
{
        ...
        if ($query->num_rows()>0) {
            ...
            if (isset($row)) {
                if ($row->accountstatus != 1) {
                    ...
                    if ($row->pwd) {
                        $_SESSION['subscriber_pwd'] = $row->pwd;
                    }
                    if ($row->email) {
                        $_SESSION['subscriber_email'] = $row->email;
                    }
                    if ($row->reg_date) {
                        $_SESSION['datecreated'] = $row->reg_date;
                    }
                    if ($row->accountstatus) {
                        $_SESSION['subscriber_status'] = $row->accountstatus;
                    }
                    ...
                    if (isset($row)) {
                        ...
                        if ($row->thumbs_bucket) {
                            $_SESSION['thumbs_bucket'] = $row->thumbs_bucket;
                        }
                    }
                    ...
                    if ($_SESSION['subscriber_name']) {
                        $nm=$_SESSION['subscriber_name'];
                    } else {
                        $nm=$_SESSION['subscriber_email'];
                    }
                    
                    if (isset($row)) {
                        if ($row->distribution_Id) {
                            $_SESSION['distribution_Id']=$row->distribution_Id;
                        }
                        if ($row->domain_name) {
                    ...
                    if ($query->num_rows()> 0) {
                        if ($row->subscribe_date) {
                            $_SESSION['subscribe_date']=$row->subscribe_date;
                        }
                        if ($row->exp_date) {
                            $_SESSION['exp_date'] = $row->exp_date;
                        }
                        if ($row->subscriptionstatus) {
                            $_SESSION['subscription_status']=$row->subscriptionstatus;
                        }
                        if ($row->plan) {
                            $_SESSION['subscriber_plan'] = $row->plan;
                        }
                    }
    ...
    public function myLogin()
    {
        ...
        if ($this->input->post('name')) {
            $fullname = $this->getdata_model->DataCleaner(trim($this->input->post('name')));
        }
        $LogDate=date('Y-m-d H:i:s');
        $remote_ip=$_SERVER['REMOTE_ADDR'];
        $remote_host=gethostbyaddr($_SERVER['REMOTE_ADDR']);
        ...
        if ($query->num_rows()>0) {
            ...
            if (isset($row)) {
                if ($row->accountstatus != 1) {
                    ...
                    if ($row->Upload_Video) {
                        $_SESSION['Upload_Video'] = $row->Upload_Video;
                    }
                    if ($row->CreateUser) {
                        $_SESSION['CreateUser'] = $row->CreateUser;
                    }
                    if ($row->SetParameters) {
                        $_SESSION['SetParameters'] = $row->SetParameters;
                    }
                    if ($row->ViewLogReport) {
                        $_SESSION['ViewLogReport'] = $row->ViewLogReport;
                    }
                    ...
                    if (isset($row)) {
                        if ($row->no_of_videos_per_day) {
                            $_SESSION['no_of_videos_per_day']=$row->no_of_videos_per_day;
                        }
                        if ($row->companyname) {
                        ...
                        if ($row->RefreshDuration) {
                            $_SESSION['RefreshDuration'] = $row->RefreshDuration;
                        }
                        if ($row->default_network) {
                            $_SESSION['default_network'] = $row->default_network;
                        }
                        if ($row->google_shortener_api) {
                            $_SESSION['google_shortener_api'] = $row->google_shortener_api;
                        }
                        if ($row->jw_api_key) {
                            $_SESSION['jw_api_key'] = $row->jw_api_key;
                        }
                        if ($row->jw_api_secret) {
                            $_SESSION['jw_api_secret'] = $row->jw_api_secret;
                        }
                        if ($row->jw_player_id) {
                            $_SESSION['jw_player_id'] = $row->jw_player_id;
                        }
                        if ($row->emergency_emails) {
                            $_SESSION['emergency_emails'] = $row->emergency_emails;
                        }
                        if ($row->emergency_no) {
                            $_SESSION['emergency_no'] = $row->emergency_no;
                        }
                        if ($row->sms_url) {
                            $_SESSION['sms_url'] = $row->sms_url;
                        }
                        if ($row->sms_username) {
                            $_SESSION['sms_username'] = $row->sms_username;
                        }
                        if ($row->sms_password) {
                            $_SESSION['sms_password'] = $row->sms_password;
                        }
                        if ($row->input_bucket) {
                            $_SESSION['input_bucket'] = $row->input_bucket;
                        }
                        if ($row->output_bucket) {
                            $_SESSION['output_bucket'] = $row->output_bucket;
                        }
                        if ($row->thumbs_bucket) {
                            $_SESSION['thumbs_bucket'] = $row->thumbs_bucket;
                        }
                        if ($row->aws_key) {
                            $_SESSION['aws_key'] = $row->aws_key;
                        }
                        if ($row->aws_secret) {
                            $_SESSION['aws_secret'] = $row->aws_secret;
                        }
                        if ($row->jwplayer_key) {
                            $_SESSION['jwplayer_key'] = $row->jwplayer_key;
                        }
                    ...
                    if (count($ret)>0) {
                        ...
                        if ($row->longlink) {
                            $_SESSION['longlink']=$row->longlink;
                        }
                        if ($row->shortlink) {
                            $_SESSION['shortlink']=$row->shortlink;
                        }
                        if ($row->pubdate) {
                            $_SESSION['pubdate']=$row->pubdate;
                        }
                        if ($row->expiredate) {
