...
class Successchargings extends CI_Controller {
	...
	public function GetReport()
	{
		...
		$sql="SELECT network,DATE_FORMAT(subscriptiondate,'%d %b %Y') AS sdate, plan,COUNT(msisdn) AS Total FROM successful_charging WHERE (TRIM(network)='".$network."') AND (DATE_FORMAT(subscriptiondate,'%Y-%m-%d') BETWEEN '".$startdate."' AND '".$enddate."') GROUP BY DATE_FORMAT(subscriptiondate,'%Y-%m-%d'),plan ORDER BY subscriptiondate DESC;";
		...
		if ($query->num_rows() > 0)
		{
			...
			$sn=0;
#$file = fopen('aaa.txt',"a"); fwrite($file, " Start \n"); fclose($file);

			while ($row = $query->unbuffered_row('array')):
				$sn++;
					
				$tp=array($sn,$row['network'],$row['sdate'],$row['plan'],number_format($row['Total'],0));			
				#$file = fopen('aaa.txt',"a"); fwrite($file, $sn." => ".$row['network']." => ".$row['sdate']." => ".$row['plan']." => ".number_format($row['Total'],0)."\n"); fclose($file);	
				$data[]=$tp;
			endwhile;
	...
	public function PrintReport()
	{
		...
		if ($query->num_rows() > 0)
		{
			...
			$xls_file='successfulchargingsreport.xls';
			...
			$objPHPExcel->getProperties()->setCreator("Eastwinds ICT")
								 ...
								 ->setKeywords("Successful Chargings,Charging")
								 ->setCategory("Successful Chargings");
			...
			$objPHPExcel->getActiveSheet()->setCellValue('A5', "S/N")
								  ...
								  ->setCellValue('C5', "Subscription Date")
			...
			$objPHPExcel->getActiveSheet()->getStyle('E'.$i)->getFill()->getStartColor()->setRGB('92D050');
			...
			$objPHPExcel->setActiveSheetIndex(0);
			$objPHPExcel->getActiveSheet()->setTitle('Successful Chargings Records');
		...
		{
			$ret="There is no new subscription record for the selected date.";
		}
	...
	{		
		...
		if ($_SESSION['username'])
		{
			...
			$this->load->view('successchargings_view',$data);
		}else
		{
			redirect("Home");
		}
