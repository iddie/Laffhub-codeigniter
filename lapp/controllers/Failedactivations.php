...
class Failedactivations extends CI_Controller {
	...
	public function GetReport()
	{
		...
		$sql="SELECT network,DATE_FORMAT(activationdate,'%d %b %Y') AS sdate, plan,COUNT(msisdn) AS Total FROM failed_activations WHERE (TRIM(network)='".$network."') AND (DATE_FORMAT(activationdate,'%Y-%m-%d') BETWEEN '".$startdate."' AND '".$enddate."') GROUP BY DATE_FORMAT(activationdate,'%Y-%m-%d'),plan ORDER BY activationdate DESC;";
...
#$file = fopen('aaa.txt',"w"); fwrite($file, $sql."\n"); fclose($file);	
	...
	public function PrintReport()
	{
		...
		if ($query->num_rows() > 0)
		{
			...
			$xls_file='failedactivationsreport.xls';
			...
			$objPHPExcel->getProperties()->setCreator("Eastwinds ICT")
								 ...
								 ->setKeywords("Failed Activations")
								 ->setCategory("Failed Activations");
			...
			$objPHPExcel->getActiveSheet()->setCellValue('A5', "S/N")
								  ...
								  ->setCellValue('C5', "Activation Date")
			...
			$objPHPExcel->setActiveSheetIndex(0);
			$objPHPExcel->getActiveSheet()->setTitle('Failed Activations Records');
		...
		{
			$ret="There is no failed activation record for the selected date.";
		}
	...
	{		
		...
		if ($_SESSION['username'])
		{
			...
			$this->load->view('failedactivations_view',$data);
		}else
		{
			redirect("Home");
		}
