		...
		if (count($nm) > 0)
		{
			...
			foreach($nm as $ph):
				if ($ph)
				{
					...
					{
						#Get plan
						$plan=$this->getdata_model->GetSubscriberPlan('',$ph);
#$file = fopen('aaa.txt',"w"); fwrite($file,$plan); fclose($file);
			...
			if (count($bad) >0)
			{
				...
				if ($cnt > 0)
				{
					$ret=array('status'=>'OK','msg'=>'Phone number(s) unsubscribed successfully. The following numbers were, however, not unsubscribed because they have already been unsubscribed: '.$s);	
				}else
				{
					$ret=array('status'=>'Failed','msg'=>'Unsubscription was not successful. The following numbers were not unsubscribed because they have already been unsubscribed: <b class="redtext">'.$s.'</b>');	
				}				
			}else
			{
				if ($cnt > 0)
				{
					$ret=array('status'=>'OK','msg'=>'Phone number(s) unsubscribed successfully.');	
				}else
				{
					$ret=array('status'=>'Failed','msg'=>'Unsubscription was not successful.');	
				}				
			}			
		}else
		{
			$ret=array('status'=>'Failed','msg'=>'Unsubscription was not successful. No phone number entered.');
		}
	...
	public function index()
	{
		...
		if ($_SESSION['username'])
		{
			...
			$this->load->view('removeuser_view',$data);
		}else
		{
			redirect("Userhome");
		}
