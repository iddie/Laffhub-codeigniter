...
class Greyareas extends CI_Controller {
	...
	public function GetReport()
	{
		...
		$sql="SELECT network,DATE_FORMAT(chargingdate,'%d %b %Y') AS sdate, plan,COUNT(msisdn) AS Total FROM greyareas WHERE (TRIM(network)='".$network."') AND (DATE_FORMAT(chargingdate,'%Y-%m-%d') BETWEEN '".$startdate."' AND '".$enddate."') GROUP BY DATE_FORMAT(chargingdate,'%Y-%m-%d'),plan ORDER BY chargingdate DESC;";
...
#$file = fopen('aaa.txt',"w"); fwrite($file, $sql."\n"); fclose($file);	
	...
	public function PrintReport()
	{
		...
		if ($query->num_rows() > 0)
		{
			...
			$objPHPExcel->getProperties()->setCreator("Eastwinds ICT")
								 ...
								 ->setKeywords("Grey Areas")
								 ->setCategory("Grey Areas");
			...
			$objPHPExcel->setActiveSheetIndex(0);
			$objPHPExcel->getActiveSheet()->setTitle('Grey Areas Records');
		...
		{
			$ret="There is no grey area record for the selected date.";
		}
	...
	{		
		...
		if ($_SESSION['username'])
		{
			...
			$this->load->view('greyareas_view',$data);
		}else
		{
			redirect("Home");
		}
