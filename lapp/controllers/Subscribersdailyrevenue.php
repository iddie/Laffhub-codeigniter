...
class Subscribersdailyrevenue extends CI_Controller {
	...
	{
		$reportdate=''; $plan='';	$network=''; $data=array();
			
		if ($this->input->post('network')) $network = trim($this->input->post('network'));
		if ($this->input->post('reportdate')) $reportdate = $this->input->post('reportdate');
		if ($this->input->post('plan')) $plan = trim($this->input->post('plan'));
		
		$sql="SELECT DATE_FORMAT(subscriptiondate,'%H:%i:%s') AS transtime,(SELECT price FROM prices WHERE (TRIM(prices.network)=TRIM(successful_charging.network)) AND (TRIM(prices.plan)=TRIM(successful_charging.plan)) LIMIT 0,1) AS price,msisdn FROM successful_charging WHERE (TRIM(network)='".$network."') AND (DATE_FORMAT(subscriptiondate,'%Y-%m-%d') = '".$reportdate."') ";
		
		if ($plan<>'') $sql .= " AND (TRIM(plan)='".$plan."')";
		$sql .= "ORDER BY subscriptiondate";
#echo $sql;
#$file = fopen('aaa.txt',"w"); fwrite($file, $sql."\n"); fclose($file);	
		...
		if ($query->num_rows() > 0)
		{
			...
			while ($row = $query->unbuffered_row('array')):
				...
				$tp=array($sn,$ph,number_format($row['price'],2),$row['transtime']);					
	...
	public function PrintReport()
	{
		...
		if ($query->num_rows() > 0)
		{
			...
			$xls_file='airtelsubscribersdailyrevenuereport.xls';
			...
			$objPHPExcel->getProperties()->setCreator("Eastwinds ICT")
									 ...
									 ->setKeywords("Subscriber,Daily,Revenue")
			...
			$objPHPExcel->getActiveSheet()->setCellValue('A5', "S/N")
								  ...
								  ->setCellValue('C5', "MSISDN")
								  ->setCellValue('D5', "Price")
								  ->setCellValue('E5', "Transaction Time");
			...
			while ($row = $query->unbuffered_row('array')):
				...
				if ($row['msisdn']) $ph=$row['msisdn'];
				if ($row['transtime']) $tm=$row['transtime'];
				...
				$objPHPExcel->getActiveSheet()->setCellValue('B' . $i, $network);
				$objPHPExcel->getActiveSheet()->setCellValue('C' . $i, $ph);
				$objPHPExcel->getActiveSheet()->setCellValue('D' . $i, $pr);
				$objPHPExcel->getActiveSheet()->setCellValue('E' . $i, $tm);					
			...
			$objPHPExcel->getActiveSheet()->getStyle('D'.$i)->getFill()->getStartColor()->setRGB('92D050');
			...
			$objPHPExcel->getActiveSheet()->getStyle('A'.$i.':E'.$i)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
			...
			$dt=date('d M Y',strtotime($reportdate));
			$objPHPExcel->getActiveSheet()->setTitle('Airtel Revenue For '.$dt);
		...
		{
			$ret="There is no Airtel subscribers daily revenue report for the selected date.";
		}
	...
	{		
		...
		if ($_SESSION['username'])
		{
			...
			$this->load->view('subscribersdailyrevenue_view',$data);
		}else
		{
			redirect("Home");
		}
