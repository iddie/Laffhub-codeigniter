...
class Newsubscription extends CI_Controller {
	...
	public function GetReport()
	{
		...
		$sql="SELECT network,DATE_FORMAT(subscriptiondate,'%d %b %Y') AS sdate, plan,COUNT(msisdn) AS Total FROM new_subscriptions WHERE (TRIM(network)='".$network."') AND (DATE_FORMAT(subscriptiondate,'%Y-%m-%d') BETWEEN '".$startdate."' AND '".$enddate."') GROUP BY DATE_FORMAT(subscriptiondate,'%Y-%m-%d'),plan ORDER BY subscriptiondate DESC;";
...
#$file = fopen('aaa.txt',"w"); fwrite($file, $sql."\n"); fclose($file);	
	...
	public function PrintReport()
	{
		...
		if ($query->num_rows() > 0)
		{
			...
			$xls_file='newsubscriptionreport.xls';
			...
			$objPHPExcel->getProperties()->setCreator("Eastwinds ICT")
									 ...
									 ->setKeywords("New Subscription,Subscription")
									 ->setCategory("New Subscription");
			...
			$objPHPExcel->setActiveSheetIndex(0);
			$objPHPExcel->getActiveSheet()->setTitle('New Subscription Records');
	...
	{		
		...
		if ($_SESSION['username'])
		{
			...
			$this->load->view('newsubscription_view',$data);
		}else
		{
			redirect("Home");
		}
