...
class Blacklistrep extends CI_Controller {
	...
	public function GetReport()
	{
		...
		$sql="SELECT network,DATE_FORMAT(blacklist_date,'%d %b %Y') AS sdate, COUNT(msisdn) AS Total FROM blacklist WHERE (TRIM(network)='".$network."') AND (DATE_FORMAT(blacklist_date,'%Y-%m-%d') BETWEEN '".$startdate."' AND '".$enddate."') GROUP BY DATE_FORMAT(blacklist_date,'%Y-%m-%d') ORDER BY blacklist_date DESC;";
...
#$file = fopen('aaa.txt',"w"); fwrite($file, $sql."\n"); fclose($file);	
		...
		{			
			...
			while ($row = $query->unbuffered_row('array')):
				...
				$tp=array($sn,$row['network'],$row['sdate'],number_format($row['Total'],0));					
	...
	public function PrintReport()
	{
		...
		if ($query->num_rows() > 0)
		{
			...
			$objPHPExcel->getProperties()->setCreator("Eastwinds ICT")
								 ...
								 ->setKeywords("Blacklist")
								 ->setCategory("Blacklist");
			...
			$objPHPExcel->getActiveSheet()->setCellValue('A5', "S/N")
								  ...
								  ->setCellValue('C5', "Blacklisted Date")
								  ->setCellValue('D5', "No. Of Subscribers");
			...
			$objPHPExcel->getActiveSheet()->getStyle('A5:D5')->getFont()->setSize(10);
												  
			$objPHPExcel->getActiveSheet()->getStyle('A5:D5')->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
			$objPHPExcel->getActiveSheet()->getStyle('A5:D5')->getFill()->getStartColor()->setRGB('FF0000');
			...
			while ($row = $query->unbuffered_row('array')):
				...
				$objPHPExcel->getActiveSheet()->setCellValue('D' . $i, $tot);		
			...
			$objPHPExcel->setActiveSheetIndex(0);
			$objPHPExcel->getActiveSheet()->setTitle('Blacklist Records');
		...
		{
			$ret="There is no blacklist record for the selected date.";
		}
	...
	{		
		...
		if ($_SESSION['username'])
		{
			...
			$this->load->view('blacklistrep_view',$data);
		}else
		{
			redirect("Home");
		}
