...
class Getdata_model extends CI_Model
{
	...
	public function GetMSISDN()
	{
		...
		return $this->GSMPhoneNo(getenv('HTTP_MSISDN'));
	}
	...
	{
		$sql = "SELECT * FROM subscriptions WHERE (TRIM(network)='".$this->db->escape_str($network)."') AND ((TRIM(msisdn)='".$this->CleanPhoneNo($this->db->escape_str($msisdn))."') OR (TRIM(email)='".$this->CleanPhoneNo($this->db->escape_str($email))."'))";
	...
	public function GetLatestVideos($email,$phone)
	{
		if ($phone)
		{
			$sql="SELECT videos.*,(SELECT rating FROM user_ratings WHERE TRIM(user_ratings.video_code)=TRIM(videos.video_code) AND (TRIM(user_ratings.msisdn)='".$phone."')) AS Rating FROM videos WHERE (play_status=1) AND (encoded=1) ORDER BY date_created DESC LIMIT 0,8";
		}elseif ($email)
		{
			$sql="SELECT videos.*,(SELECT rating FROM user_ratings WHERE TRIM(user_ratings.video_code)=TRIM(videos.video_code) AND (TRIM(user_ratings.email)='".$email."')) AS Rating FROM videos WHERE (play_status=1) AND (encoded=1) ORDER BY date_created DESC LIMIT 0,8";
		}
		...
		if ($videolist <> '')
		{
			...
			if (count($arrWatched)>0)
			{
				...
				if ($list <> '')
				{
					...
					$this->db->where(array('subscriptionId'=>$subscriptionId));
	...
	public function SetWatchCount($videocode,$phone,$email,$subscriptionId)
	{
		...
		$this->db->where(array('video_code'=>$videocode));
...
#$file = fopen('aaa.txt',"w"); fwrite($file,$dt."\nExp. Date=".$expdt."\nRet=".$ret); fclose($file);		
	...
	public function UpdateSubscriptionStatus($email,$phone,$status)
	{
		...
		if ($query->num_rows() > 0 )#Update
		{
			...
			$this->db->or_where('email =', $email);  // Produces: WHERE name != 'Joe' OR id > 50
