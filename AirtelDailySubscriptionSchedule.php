	...
	{	
		$table='<table style="border:solid thin;" cellpadding="5" cellspacing="0">
			<tr bgcolor="#eeeeee">
				...
				<th align="center" style="border:solid thin;">NO OF SUBSCRIBERS</th>
			</tr>
		';
		...
		foreach($plans as $plan):
			...
			$sql="SELECT COUNT(DISTINCT msisdn) AS SubscriberCount FROM `subscription_history` WHERE (DATE_FORMAT(`subscribe_date`,'%Y-%m-%d')='".$dt."') AND (TRIM(plan)='".$plan."') AND (TRIM(subscription_status)='OK') ";
			...
			$table .= 
				'<tr>
					...
					<td align="center" style="border-style:solid; border-width:thin;">'.number_format($cnt,0).'</td>
				</tr>';
		...
		$table .='
			<tr bgcolor="#eeeeee">
				<th colspan="2" align="center" style="border:solid thin;">TOTAL SUBSCRIBERS</th>
		...
		$subject='Airtel LaffHub Daily Subscription Report For '.$rdt;
		$Cc='idongesit_a@yahoo.com,nsikakj@gmail.com';
		
		#$to='idongesit_a@yahoo.com';
		#$Cc='nsikakj@gmail.com';
		...
		$message='
			...
			Below is Airtel Laffhub subscription report for '.$rdt.':<br><br>'.
			$table.'<br>
