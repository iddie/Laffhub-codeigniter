...
class Getdata_model extends CI_Model
{
	...
	public function GetMSISDN()
	{
		...
		return strip_tags($this->CleanPhoneNo(getenv('HTTP_X_UP_CALLING_LINE_ID')));
	}
...
#$file = fopen('aaa.txt',"w"); fwrite($file,$sql); fclose($file);	
	...
	{
		#Get Popular Movies ID
		$sql="SELECT * FROM videos WHERE (play_status=1) AND (encoded=1) ORDER BY watchcount DESC LIMIT 0,7";
	...
	public function GetLatestVideos($email,$phone)
	{
		$sql="SELECT videos.*,(SELECT rating FROM user_ratings,videos WHERE (TRIM(user_ratings.video_code)=TRIM(videos.video_code)) AND (TRIM(user_ratings.msisdn)='".$phone."') LIMIT 0,1) AS Rating FROM videos WHERE (play_status=1) AND (encoded=1) ORDER BY date_created DESC LIMIT 0,8";
		...
		if ($videolist <> '')
		{
			...
			if (count($arrWatched)>0)
			{
				foreach($arrWatched as $code => $cnt): #foreach (array_expression as $key => $value)
					if ($code)
					{
						if (trim($code) == trim($videocode)) $cnt++;						
						if ($list=='') $list=$code.'|'.$cnt; else $list .= '^'.$code.'|'.$cnt;
					}
				endforeach;
...
#$file = fopen('aaa.txt',"w"); fwrite($file,$dt."\nExp. Date=".$expdt."\nRet=".$ret); fclose($file);		
	...
	public function GetVideoComments($category,$videocode,$filename)
	{
		$sql = "SELECT * FROM comments WHERE (TRIM(category)='".$this->db->escape_str($category)."') AND ((TRIM(videocode)='".$this->db->escape_str($videocode)."') OR ((TRIM(filename)='".$this->db->escape_str($filename)."'))) AND (commentstatus=1) ORDER BY commentdate DESC LIMIT 0,25";
