...
class V extends CI_Controller
{
    ...
    public function LikeComment()
    {
        ...
        $sql = "SELECT * FROM comments_likes WHERE (TRIM(msisdn)='".$this->db->escape_str($phone)."') AND (TRIM(comment_id)='".$this->db->escape_str($comment_id)."')";
        ...
        if ($query->num_rows() == 0) {
            ...
            if ($this->db->trans_status() === false) {
                $Msg="Subscriber with msisdn ".$phone.' ('.strtoupper($author).") attempted liking comment ".strtoupper($comment)." but failed.";
            } else {
                $likes++;
                ...
                $Msg="Subscriber with msisdn ".$phone.' ('.strtoupper($author).") liked the comment ".strtoupper($comment)." successfully.";
            ...
            $this->getdata_model->LogDetails($phone, $Msg, $auhor, date('Y-m-d H:i:s'), $remote_ip, $remote_host, 'LIKED COMMENT', $_SESSION['LogID']);
        }
    ...
    public function DislikeComment()
    {
        ...
        if ($query->num_rows() == 0) {
            ...
            if ($this->db->trans_status() === false) {
                $Msg="Subscriber with msisdn ".$phone.' ('.strtoupper($author).") attempted disliking comment ".strtoupper($comment)." but failed.";
            } else {
                $dislikes++;
                ...
                $Msg="Subscriber with msisdn ".$phone.' ('.strtoupper($author).") disliked the comment ".strtoupper($comment)." successfully.";
            ...
            $this->getdata_model->LogDetails($phone, $Msg, $auhor, date('Y-m-d H:i:s'), $remote_ip, $remote_host, 'DISLIKED COMMENT', $_SESSION['LogID']);
        }
    ...
    public function DislikeVideo()
    {
        ...
        if ($this->input->post('videotitle')) {
            $videotitle = trim($this->input->post('videotitle'));
        }
        if ($this->input->post('videocode')) {
            $videocode = trim($this->input->post('videocode'));
        }
                        
        //Check if record exists
        $sql = "SELECT * FROM likes WHERE (TRIM(msisdn)='".$this->db->escape_str($phone)."') AND (TRIM(video_code)='".$this->db->escape_str($videocode)."')";
        ...
        if ($query->num_rows() == 0) {
            ...
            if ($this->db->trans_status() === false) {
                $Msg="Subscriber with msisdn ".$phone." attempted disliking video ".$videotitle.'('.$videocode.") but failed.";
                ...
                $Msg="Subscriber with msisdn ".$phone." disliked the video ".$videotitle.'('.$videocode.") successfully.";
            ...
            $this->getdata_model->LogDetails($phone, $Msg, $phone, date('Y-m-d H:i:s'), $remote_ip, $remote_host, 'DISLIKED VIDEO', $_SESSION['LogID']);
        } else {
            $ret='';
        }
    ...
    public function LikeVideo()
    {
        ...
        if ($query->num_rows() == 0) {
            ...
            if ($this->db->trans_status() === false) {
                $Msg="Subscriber with msisdn ".$phone." attempted liking video ".$videotitle.'('.$videocode.") but failed.";
                ...
                $Msg="Subscriber with msisdn ".$phone." liked the video ".$videotitle.'('.$videocode.") successfully.";
            ...
            $this->getdata_model->LogDetails($phone, $Msg, $phone, date('Y-m-d H:i:s'), $remote_ip, $remote_host, 'LIKED VIDEO', $_SESSION['LogID']);
        } else {
            $ret='';
        }
    ...
    public function AddComment()
    {
        ...
        if ($query->num_rows() > 0) {
            ...
            $this->getdata_model->LogDetails($author, $Msg, $msisdn, date('Y-m-d H:i:s'), $remote_ip, $remote_host, 'ADDED COMMENT', $_SESSION['LogID']);
        }
        ...
        if (strtolower(trim($ret))=='airtel') {
            ...
            if ($rt==true) {#Blacklisted
                ...
                if ($this->uri->segment(1)) {
                    $videocode=trim(str_replace('c-', '', $this->uri->segment(1)));
                }
                ...
                #Get Player Key
                if ($_SESSION['jwplayer_key']) {
                    $data['jwplayer_key'] = $_SESSION['jwplayer_key'];
                }
                if ($_SESSION['thumbs_bucket']) {
                    $ThumbBucket = $_SESSION['thumbs_bucket'];
                }
                ...
                if ($query->num_rows() == 0) {#New User
                    $isnew=$this->getdata_model->IsNewSubscriber($phone, $network);
                    ...
                    if (intval($isnew, 10)==1) {
                        ...
                        $sql="SELECT msisdn FROM new_subscriptions WHERE (TRIM(network)='".$this->db->escape_str($network)."') AND (TRIM(msisdn)='".$this->db->escape_str($phone)."') AND (DATE_FORMAT(subscriptiondate,'%Y-%m-%d')='".date('Y-m-d')."')";
                        ...
                        if ($query->num_rows() > 0) {
                            ...
                            $where = "(TRIM(network)='".$this->db->escape_str($network)."') AND (TRIM(msisdn)='".$this->db->escape_str($phone)."') AND (DATE_FORMAT(subscriptiondate,'%Y-%m-%d')='".date('Y-m-d')."')";
                        ...
                        if ($msisdn) {
                            ...
                            $sql = "SELECT subscription,fallback_notice FROM subscriber_messages WHERE (TRIM(network)='".$this->db->escape_str($network)."') AND (TRIM(plan)='Free Trial') ";
                            ...
                            if ($query->num_rows() > 0) {
                                $row = $query->row();
                                if ($row->subscription) {
                                    $sentmessage=$row->subscription;
                                }
                                
                                if (!$sentmessage) {
                                    if ($row->fallback_notice) {
                                        $sentmessage=$row->fallback_notice;
                                    }
                                }
                            }
                            
                            if (!$sentmessage) {
                                $sentmessage='You have been charged N'.$amount.' for '.$plan.' Laffhub service. Visit www.laffhub.com. NO DATA COST. To opt out, text OUT to 2001.';
                            }
                ...
                if ($query->num_rows() > 0) {
                    ...
                    if ($row->subscriptionId) {
                        $subscriptionId=trim($row->subscriptionId);
                    }
                    if ($row->videos_cnt_to_watch) {
                        $MaxVideo=trim($row->videos_cnt_to_watch);
                    }
                    ...
                    if ($row->email) {
                        $SubscriberEmail=trim($row->email);
                    }
                    
                    if (strtolower($MaxVideo)=='unlimited') {
                        $MaxVideo='1000000000';
                    }
                    ...
                    if (is_array($result)) {
                        foreach ($result as $rw) {
                            if ($rw->subscribe_date) {
                                $dt = date('F d, Y', strtotime($rw->subscribe_date));
                            }
                            
                            $data['subscribe_date'] = $dt;
                            
                            if ($rw->exp_date) {
                                $edt = date('F d, Y', strtotime($rw->exp_date));
                            }
                            $data['exp_date'] = $edt;
                    ...
                    if ($query->num_rows() > 0) {
                        ...
                        if ($row->videolist) {
                            $videolist=trim($row->videolist);
                        }
                        
                        if ($videolist <> '') {
                            $VideosWatched=count(explode('^', $videolist));
                        } else {
                            $VideosWatched=0;
                        }
                    } else {
                    ...
                    if ($query->num_rows() > 0) {
                        ...
                        if ($row->watchcount) {
                            $data['watchcount']=$row->watchcount;
                        }
                                                
                        if ($row->comedian) {
                            if (strtolower($row->comedian)<>'undefined') {
                    ...
                    #Check if current video maximum count of 5 has been reached
                    ...
                    if ($this->db->trans_status() === false) {
                        ...
                        $this->getdata_model->LogDetails($data['Phone'], $Msg, 'System', date('Y-m-d H:i:s'), $remote_ip, $remote_host, 'NEW WATCH VIDEO REQUEST', 'System');
                    }
    ...
    public function CheckForWatchCount()
    {
        ...
        if ($this->input->post('subscriptionId')) {
            $subscriptionId = trim($this->input->post('subscriptionId'));
        }
