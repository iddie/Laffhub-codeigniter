...
class Publishershow extends CI_Controller {
    ...
    {
        $publisher=''; $page=1; $rowcount=20;

        if ($this->input->post('page')) $page = $this->input->post('page');
        if ($this->input->post('publisher')) $publisher = $this->input->post('publisher');
        ...
        }else
        {
            ...
            {
                #Get Publisher videos

                $sql = "SELECT videos.*, publishers.publisher_name FROM videos LEFT JOIN publishers on videos.publisher_email = publishers.publisher_email
                        WHERE (videos.play_status=1) AND (videos.encoded=1) AND (TRIM(publishers.publisher_name)='".$this->db->escape_str($publisher)."')";

                $order = " ORDER BY videos.video_title";
                ...
                if ($query->num_rows() > 0 )
                {
                    $data['PublisherVideos']=$query->result();
                    ...
                    $data['Publisher']=urldecode($publisher);
...
//                    $data['ComedianId']=$id;
    ...
    {
        $id='';  $page=1; $rowcount=20; $publisher_name='';

        if ($this->uri->segment(3)) $id=$this->uri->segment(3);
        if ($this->input->post('page')) $page = $this->input->post('page');

        $sql = "SELECT * FROM publishers WHERE (id=".$this->db->escape_str($id).")";
        ...
        if ($query->num_rows() > 0 )
        {
            ...
            if ($row->publisher_name) $publisher_name=trim($row->publisher_name);
            ...
            }else
            {
                ...
                {
                    #Get Publisher videos
                    $sql_publisher = "SELECT videos.*, publishers.publisher_name FROM videos LEFT JOIN publishers on videos.publisher_email = publishers.publisher_email
                        WHERE (videos.play_status=1) AND (videos.encoded=1) AND (TRIM(publishers.publisher_name)='".$this->db->escape_str($publisher_name)."')";

                    $order = " ORDER BY videos.video_title";

                    $sql=$this->getdata_model->GetPaginationUrl($page,$rowcount,$sql_publisher,$order);
                    ...
                    if ($query->num_rows() > 0 )
                    {
                        ...
                        $data['ActiveAdverts']=$this->getdata_model->GetActiveAdverts();
                        $data['Categories']=$this->getdata_model->GetCategories();
                        $data['Publisher']=urldecode($publisher_name);
                        $data['PublisherId']=$id;
    ...
    public function index()
    {
        ...
        $data['ActiveAdverts']=$this->getdata_model->GetActiveAdverts();
        $data['Categories']=$this->getdata_model->GetCategories();
        $this->load->view('publisher_view',$data);#Fail Page
    }
}
