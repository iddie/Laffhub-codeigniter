...
class Videocat extends CI_Controller {	
	function __construct() 
	{
		...
		$this->load->model('videocat_model');
	 }
	
	public function LoadVideoCategoriesJson()
	{
				
		$ret=$this->videocat_model->GetVideoCategories();
		...
		{			
			foreach($ret as $row):
				$tp=array('<img  style="cursor:pointer" src="'.base_url().'images/view_icon.png" height="15" title="Select '.strtoupper($row->category).'\'s Record">',$row->category,$row->id);
				$data['data'][]=$tp;
			endforeach;
		}
