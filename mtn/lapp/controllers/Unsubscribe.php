...
class Unsubscribe extends CI_Controller {	
	...
	public function UnsubscribeUser()
	{
		...
		$exist=trim($this->getdata_model->MTNCheckStatus($msisdn));
		
		if (strtolower($exist) == 'active')
		{
			$val=strtoupper($this->getdata_model->MTNUnsubscribe($msisdn));
			...
			if ($val=='OK')
			{
				if ($query->num_rows() > 0 )#There is active subscription
				{
					...
					#CUSTOMERS WITH FAILED CHARGINGS
					$sql = "SELECT * FROM mtn_daily_revenue WHERE DATE_FORMAT(subscribe_date,'%Y-%m-%d')='".date('Y-m-d')."'";
					...
					if ($query->num_rows() > 0)
					{
						...
						$this->db->update('mtn_daily_revenue', $dat); 	
					...
					}else
					{
						...
						$this->db->insert('mtn_daily_revenue', $dat); 	
					...
					$Msg='Subscriber with MSISDN, '.$msisdn.', has unsubscribed from Laffhub service successfully.';
					...
					$this->getdata_model->LogDetails($network.' LaffHub',$Msg,$msisdn,$_SESSION['LogIn'],$_SESSION['RemoteIP'],$_SESSION['RemoteHost'],'UNSUBSCRIBED USER',$_SESSION['LogID']);
				}else
				{
					$ret="Your attempt to unsubscribe from Laffhub ".strtoupper($plan)." plan failed. You have no active subscription on Laffhub service.<br>Text <b>LF to 700</b> to activate 7dys/15 videos. Service costs &#8358;50/Week";
				}			
			}elseif ($val=='ERROR')
			{
				$ret="Unsubscription was not successful. <b>".$val."</b>";
			}	
		}else
		{
			$ret="Unsubscription was not successful. Subscriber does not have an active subscription running.";
		}
	...
	public function index()
	{
		...
		$result=$this->getdata_model->GetSubscriptionDetails($data['Network'],$data['Phone']);
		...
		if (count($result)>0)
		{
			foreach($result as $row)
			{
				...
				if ($row['videos_cnt_to_watch']) $data['videos_cnt_to_watch'] = number_format($row['videos_cnt_to_watch'],0);
