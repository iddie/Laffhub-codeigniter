...
class Subscribe extends CI_Controller
{
    ...
    public function LoadSubscriptionHistory()
    {
        ...
        if ($query->num_rows() > 0) {
            ...
            if ($row['subscribe_date']) {
                $sdt=date('d M Y @ H:i', strtotime($row['subscribe_date']));
            }
            if ($row['subscription_expiredate']) {
                $edt=date('d M Y @ H:i', strtotime($row['subscription_expiredate']));
            }
            if ($row['subscription_status']) {
                $sta=strtolower(trim($row['subscription_status']));
            }
    ...
    public function SubscribeUser()
    {
        ...
        if ($this->input->post('autobilling')) {
            $autobilling = trim($this->input->post('autobilling'));
        }
        if ($this->input->post('amount')) {
            $amount = trim($this->input->post('amount'));
        }
        if ($this->input->post('subscribe_date')) {
            $subscribe_date = $this->input->post('subscribe_date');
        }
        if ($this->input->post('exp_date')) {
            $exp_date = trim($this->input->post('exp_date'));
        }
        if ($this->input->post('subscriptionId')) {
            $subscriptionId = trim($this->input->post('subscriptionId'));
        }
        if ($this->input->post('videos_cnt_to_watch')) {
            $videos_cnt_to_watch = trim($this->input->post('videos_cnt_to_watch'));
        }
        if (!$autobilling) {
            $autobilling='0';
        }
        ...
        if ($rt==true) {
            ...
            if ($query->num_rows() > 0) {#There is active subscription
                ...
                if ($row->plan) {
                    $activeplan = trim($row->plan);
                }
                if ($row->videos_cnt_watched) {
                    $watched = intval($row->videos_cnt_watched);
                }
                if ($row->videos_cnt_to_watch) {
                    $maxwatch = $row->videos_cnt_to_watch;
                }
    ...
    public function index()
    {
        ...
        if ($_SESSION['subscriber_status']) {
            $data['subscriber_status'] = $_SESSION['subscriber_status'];
        }
        ...
        if (is_array($result)) {
            ...
            foreach ($result as $row) {
                if ($row->subscribe_date) {
                    $dt = date('F d, Y', strtotime($row->subscribe_date));
                }

                $data['subscribe_date'] = $dt;

                if ($row->exp_date) {
                    $edt = date('F d, Y', strtotime($row->exp_date));
                }
                $data['exp_date'] = $edt;
        ...
        if (count($PayStackSettings)>0) {
            ...
            if ($row->payment_currency) {
                $data['payment_currency']=$row->payment_currency;
            }
