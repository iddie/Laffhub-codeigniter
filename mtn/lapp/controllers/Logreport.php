...
class Logreport extends CI_Controller {
	...
	public function GetAuditTrail()
	{
		...
		$sql="SELECT DATE_FORMAT(ActionDate,'%d&nbsp;%b&nbsp;%Y&nbsp;%H:%i') AS ActionDate,`name`,Activity,Username,Operation,remote_ip,remote_host FROM loginfo";
		...
		if ((trim($startdate) != '') && (trim($enddate) != ''))
		{
			if (trim($crit)=='')
			{
				$crit=" (DATE_FORMAT(ActionDate,'%Y-%m-%d') BETWEEN '".$startdate."' AND '".$enddate."')";
			}else
			{
				$crit .= " AND (DATE_FORMAT(ActionDate,'%Y-%m-%d') BETWEEN '".$startdate."' AND '".$enddate."')";
			}
		}
		
		if (trim($crit) != '') $sql .= ' WHERE'.$crit;
		
		$sql .= " ORDER BY ActionDate,`name`";
...
#$file = fopen('aaa.txt',"w"); fwrite($file, $sql."\n"); fclose($file);	
		...
		{
			$data=array();
//ActionDate,Username,Fullname,Operation,Activity,remote_ip,remote_host
			if (is_array($results))
			{
				foreach($results as $row):
					$tp=array($row['ActionDate'],$row['Username'],$row['name'],$row['Operation'],str_replace("\'","'",html_entity_decode($row['Activity'])),$row['remote_ip'],$row['remote_host']);
					$data[]=$tp;
				endforeach;
			}
...
#[Jan] => Array([0] => [1] => 7 [2] => 12 [3] => 8) [Feb] => Array([0] => 7 [1] => 9 [2] => 5 [3] => 9)
	...
	{		
		if ($_SESSION['username'])
		{
			...
			$this->load->view('logreport_view',$data);
		}else
		{
			redirect("Home");
		}
