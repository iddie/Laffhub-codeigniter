...
class Getdata_model extends CI_Model
{
	...
	public function UpdateMTNSubscriptionStatus($plan,$network,$phone,$status)
	{
				
		$sql="SELECT * FROM subscriptions WHERE (TRIM(network)='".$this->db->escape_str($network)."') AND (TRIM(msisdn)='".$this->db->escape_str($phone)."')";
		...
		{
			$dat=array('subscriptionstatus' => $status, 'plan' => $plan);
				
			$where = "(TRIM(network)='".$this->db->escape_str($network)."') AND (TRIM(msisdn)='".$this->db->escape_str($phone)."')";
	...
	#Create new account
	public function CreateNewMTNAccount($network,$phone,$date,$plan,$duration)
	{
		##################################################				
		#### Update Into new_subscriptions
		$tdt=date("Y-m-d H:i:s");
		
		$sql="SELECT msisdn FROM new_subscriptions WHERE (TRIM(network)='".$this->db->escape_str($network)."') AND (TRIM(msisdn)='".$this->db->escape_str($phone)."')";
		...
		$msisdn=$phone;
		$videos_cnt_to_watch = 30;	$amount=0;			$autobilling=1;
		$subscribe_date = $tdt;
		...
		$this->getdata_model->LogDetails($msisdn,$Msg,'System',date('Y-m-d H:i:s'),$remote_ip,$remote_host,'NEW SUBSCRIPTION ACCOUNT CREATED','System');
	...
	public function CheckForBlackList($network,$phone)
	{
		$sql = "SELECT * FROM blacklist WHERE (TRIM(network)='".$this->db->escape_str($network)."') AND (TRIM(msisdn)='".$this->db->escape_str($phone)."')";
	...
	{
		//return '2347036520964'; #REMOVE LATER
		...
		}elseif (getenv('HTTP_X_UP_CALLING_LINE_ID'))
		{
		...
		}elseif (getenv('HTTP-MSISDN'))
		{
			$ph=strip_tags($this->CleanPhoneNo(getenv('HTTP-MSISDN')));
		}
	...
	{
		$sql="SELECT DATE_FORMAT(subscribe_date,'%Y-%m-%d') AS 'dt', COUNT(msisdn) AS Total FROM subscriptions WHERE (TRIM(network)='".$this->db->escape_str($network)."') AND (TRIM(plan)='Trial') AND (DATE_FORMAT(subscribe_date,'%Y-%m-%d') BETWEEN '".$startdate."' AND '".$enddate."') GROUP BY DATE_FORMAT(subscribe_date,'%Y-%m-%d') ORDER BY DATE_FORMAT(subscribe_date,'%Y-%m-%d') DESC";
	...
	{
		$sql="SELECT DATE_FORMAT(subscribe_date,'%Y-%m-%d') AS 'dt', COUNT(msisdn) AS Total FROM subscriptions WHERE (TRIM(network)='".$this->db->escape_str($network)."') AND (DATE_FORMAT(subscribe_date,'%Y-%m-%d') BETWEEN '".$startdate."' AND '".$enddate."') GROUP BY DATE_FORMAT(subscribe_date,'%Y-%m-%d') ORDER BY DATE_FORMAT(subscribe_date,'%Y-%m-%d') DESC";
	...
	public function LoadSubscriberSession($msisdn)
	{
		...
		$_SESSION['SubscriberPhone']=$msisdn;
		...
		$sql="SELECT * FROM subscriptions WHERE (TRIM(msisdn)='".$this->db->escape_str($msisdn)."')";
...
#$file = fopen('aaa.txt',"w"); fwrite($file,$sql); fclose($file);	
		...
		if ($query->num_rows()>0)
		{
			...
			if ($row->subscriptionstatus) $_SESSION['subscriber_status'] = $row->subscriptionstatus;
			if ($row->plan) $_SESSION['subscriber_plan'] = $row->plan;
								
			$_SESSION['LogID']=uniqid();						
			
									
			$this->getdata_model->LogDetails($msisdn,'Subscriber Login',$msisdn,$LogDate,$remote_ip,$remote_host,'SUBSCRIBER LOGIN',$_SESSION['LogID']);					
		}
	...
	public function LoadRegularSession($msisdn)
	{
		$_SESSION['LogID']='';
		$_SESSION['subscriber_email']='';
		$_SESSION['subscriber_name']='';
...
#$file = fopen('aaa.txt',"w"); fwrite($file,$sql); fclose($file);	
	...
	public function GetSubscriptionDetails($network,$msisdn)
	{
		$sql = "SELECT * FROM subscriptions WHERE (TRIM(network)='".$this->db->escape_str($network)."') AND (TRIM(msisdn)='".$this->CleanPhoneNo($this->db->escape_str($msisdn))."')";
		...
		{			
			...
			if (count($arrSubscriber) > 0)
			{
				foreach ($arrSubscriber as $sub):
					if ($sub)
					{
						$sql="SELECT video_code,rating FROM user_ratings WHERE (TRIM(msisdn)='".$this->db->escape_str($sub)."') AND (rating>0)";
	...
	public function MTNCheckStatus($msisdn)
	{
		...
		if ($query->num_rows() > 0 )
		{
            ...
            if ($row['url']) $apiurl=trim($row['url']);
            if ($row['channel']) $channel=trim($row['channel']);
            if ($row['svcid']) $svcid=trim($row['svcid']);
            if ($row['spid']) $spid=trim($row['spid']);
						
			try
			{
                $url=$apiurl.'?msisdn='.$msisdn.'&req=CHECK&svcid='.$svcid.'&channel='.$channel.'&spid='.$spid;
				...
				$result = curl_exec($ch); #Send the request & save response to $resp
				
				curl_close($ch); #Close request to clear up some resources
				
				$SVCID = ""; $SVCDESC = ""; $STATUS = ""; $DESC = ""; $msg=''; $code='';

				$simpleXMLElement = simplexml_load_string($result);
				$SVCID = (string)$simpleXMLElement->SERVICE->SVCID;
				
				if (!$SVCID)
				{
					return 'Not Active';	
				}elseif (strtoupper($SVCID)=='LF')
				{#SVCDESC=LAFFHUB
					$SVCDESC = (string)$simpleXMLElement->SERVICE->SVCDESC;
					$STATUS = (string)$simpleXMLElement->SERVICE->STATUS;
					$DESC = (string)$simpleXMLElement->SERVICE->DESC;					
					
					if (strtoupper($STATUS)=='ACTIVE')#There is active subscription
					{# Subscription
						return 'ACTIVE';
					}elseif(strtoupper($STATUS) == 'DEACTIVE'){
	...
	public function MTNUnsubscribe($msisdn)
	{
		...
		if ($query->num_rows() > 0 )
		{
			...
			try
			{
				$url=$apiurl.'?msisdn='.$msisdn.'&req=UNSUB&svcid='.$svcid.'&channel='.$channel;
				...
				{#SVCDESC=LAFFHUB
					...
					{#Active Subscription
						...
						return  $msg;
					}elseif (strtoupper($STATUS)=='UNSUB')#Unsubscribed
					{#NO DESC - description
						return 'OK';	
					}elseif (strtoupper($STATUS)=='ERROR')#ERROR
					{#Has DESC - description
						return 'ERROR: '.$DESC;
					}
				}				
	...
	public function MTNBilling($msisdn)
	{
		$channel = ''; $apiurl = ''; $svcid=''; $spid='';
		...
		if ($query->num_rows() > 0 )
		{
			...
			try
			{
				$url=$apiurl.'?msisdn='.$msisdn.'&req=SUB&svcid='.$svcid.'&channel='.$channel.'&spid='.$spid;
				...
				}elseif (strtoupper($SVCID)=='LF') {#SVCDESC=LAFFHUB

                    $SVCDESC = (string)$simpleXMLElement->SERVICE->SVCDESC;
                    $STATUS = (string)$simpleXMLElement->SERVICE->STATUS;
                    $DESC = (string)$simpleXMLElement->SERVICE->DESC;

                    //Subscription is being processed. Confirmation POPUP sent to customer
                    if (strpos($STATUS, 'PENDING|202|') !== false) {
                        $msg = 'Processing';
                        return $msg;

                    } elseif (strpos($STATUS, 'ERROR|207|') !== false) {
                        $msg = 'Pending Request';
                        return $msg;

                    } elseif ((strpos($STATUS, 'ERROR|91|SM0091') !== false)) {
                        $msg = 'Already Active';  #Already active customer
                        return $msg;
                    } else {
                        $msg = "Error Couldn't process request";
                        return $msg;
                    }
                }
		...
		{
			$msg='No MTN Settings Record';
			$code='E2';
			return array('Status' => 'FAILED','errorCode' => $code,'errorMessage' => $msg);
		}
	}
			 
	public function GetComedianVideos($comedian,$count=24)
	{
		if ($count=='')
		{
			$sql = "SELECT * FROM videos WHERE (TRIM(comedian)='".$this->db->escape_str($comedian)."') AND (play_status=1) LIMIT 0,6";
		}else
		{
	...
	public function CheckSubscriptionDate($email,$phone)
	{
		...
		$sql="SELECT exp_date FROM subscriptions WHERE (TRIM(msisdn)='".$this->db->escape_str($phone)."')";
		...
		if ( $query->num_rows()> 0 )
		{
			...
			{
				$ret='1';
				$this->UpdateSubscriptionStatus($email,$phone,'1');
			}else
			{
	...
	public function GenerateMsgRSS($schedule_id)
	{
		...
		if ($schedule_id)
		{
			...
			{			
				...
				$data .='<image>';
				$data .='<url>http://laffhub.com/img/logo.jpg</url>';
				$data .='</image>';	
				$data .= '</channel>';
				$data .= '</rss> ';
...
#$file = fopen('aaa.txt',"w"); fwrite($file, $sql."\nOld Fees ID=".$OldFeed."\nOld Schedule ID=".$OldScheduleID."\nNew ID=".$new_id); fclose($file);		
	...
	{
		//Url to shorten
		#$longUrl = 'http://www.brandstolife.com';
		#Get API Key
		...
		if (count($ret)>0)
		{
			...
			if ($user && $pass)
			{
				...
				if (floatval($ret)<100)#Send message
				{
					...
					#Send SMS
					$this->SendBulkSMS('LaffHub',$this->CleanPhoneNo($phones),$m);
				}
